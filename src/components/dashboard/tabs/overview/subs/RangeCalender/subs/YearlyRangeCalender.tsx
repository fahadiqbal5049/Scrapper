...
interface YearlyRangeCalenderProps {
  isOpen: boolean;
  value: DateObject[];
...
const YearlyRangeCalender: React.FC<YearlyRangeCalenderProps> = ({
  isOpen,
  value,
  onChange,
}) => {
  ...
  const formattedValues = value?.map((date) => date.format("MMM YYYY"));

  return (
    <div
      className="flex select-none yearly-range py-3 relative flex-col"
      ref={containerRef}
    >
...
export default YearlyRangeCalender;
